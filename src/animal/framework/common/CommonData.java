...
public List<HashMap> fileUpLoadMulti( MultipartHttpServletRequest multipartRequest ,  String explainCd ) throws IllegalStateException, IOException{
		...
		List<HashMap> fileAddMapList = new ArrayList<HashMap>();	//리턴할 리스트
	...
	public DataMap recMultiMap(String multiCd, String[] multiVal, DataMap dataMap){
		...
		System.out.println("###############################################################");
		System.out.println("######################### 그룹코드에 속한 코드: "+Arrays.toString(multiVal));
	...
	public void insertRecode( DataMap dataMap ) throws IllegalStateException, IOException{
		...
		cal.set ( year2,month2,day2 );// 오늘로 설정. 
		Calendar cal2 = Calendar.getInstance ( );
		cal2.set ( year1, month1, day1 ); // 기준일로 설정. month의 경우 해당월수-1을 해줍니다.
		...
		while ( !cal2.after ( cal ) ){			
			...
			count++;cal2.add ( Calendar.DATE, 1 ); // 다음날로 바뀜
		}
	}	
	
	/**
	 * 선발시 일지 테이블에 추가된 실습일수 만큼 로우 생성
	 * @param request
	 * @throws IllegalStateException
	 * @throws IOException
	 */
	...
	//	cal2.add(Calendar.DATE, 1); //다음날부터 등록되게 설정
	...
	public void insertAddWeek( DataMap dataMap ) throws IllegalStateException, IOException{
		...
		cal3.set ( year2,month2,day2 );// 오늘로 설정.
		...
		while ( !cal2.after ( cal ) ){			
			...
			if(day_of_week == 6){
				weekcnt++;
				exeView.put("WEEK_SEQ", weekcnt);	//주별시컨스
				exeView.put("WEEK_ST_DT", week_st_dt);	//주간시작일
				exeView.put("WEEK_ED_DT", week_ed_dt);	//주간종료일
				...
				if(dFormat.format(cal3.getTime()).equals(dFormat.format(cal2.getTime()))){
					...
					exeView.put("WEEK_SEQ", weekcnt);	//주별시컨스
					exeView.put("WEEK_ST_DT", week_st_dt);	//주간시작일
					exeView.put("WEEK_ED_DT", dFormat.format(cal2.getTime()));	//주간종료일
					exeView.put("procedureid", "Comp.insertWeek");			
					this.commonFacade.processInsert(exeView);
				}	
			}
	...
	//이력서 사진올리기
		public HashMap getResumeFileUpLoad( MultipartHttpServletRequest multipartRequest , String destinationDir ) throws IllegalStateException, IOException{
			...
			while(fileNameIter.hasNext()) {
				...
				if(uploadFile != null){
					...
					String onriginFileName = uploadFile.getOriginalFilename().trim();			
					System.out.println("파일업로드 이름 ="+onriginFileName);
					System.out.println("파라미터 이름 ="+uploadFile.getName());
					String filePathTemp = "";
		...
		public void delteChk( String NO ,String TableNm , String columnm ){
			LogWriter.getLogger(getClass()).debug("******************* CommonData    하나씩 삭제  ************************* " );
			LogWriter.getLogger(getClass()).debug("***** 지우려는 컬럼 번호  ***** " +NO);
			LogWriter.getLogger(getClass()).debug("*******지우려는 테이블명 ***** " +TableNm);
			LogWriter.getLogger(getClass()).debug("*******지우려는 컬럼명 ***** " +columnm);
		...
		public DataMap setPageValue(DataMap dataMap){
			...
			int totalCnt = dataMap.getInt("TOTAL_CNT");												//총갯수
			int currPage = dataMap.getInt("CURR_PAGE") == 0 ? 1 : dataMap.getInt("CURR_PAGE");		//현재페이지
			int pageSize = dataMap.getInt("PAGE_SIZE") == 0 ? 10 : dataMap.getInt("PAGE_SIZE");		//한페이지당 갯수
