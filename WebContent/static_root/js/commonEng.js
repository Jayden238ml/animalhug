...
function fnFileUpload(LIMIT_FILE_CNT) {
	...
	window.open("/common/index.do?jPath=swfUpload/swfPopupEng&LIMIT_FILE_CNT="+LIMIT_FILE_CNT
...
function elementCheck(V_FORM) {
	...
	$form.find("INPUT,TEXTAREA,SELECT").each(function () {
		...
		if ( $element.attr('required') == 'required') {
			if (value == '') {
				alert(title + " Is a required input.");
		...
		if ( $element.attr('numeric') == 'numeric') {
			...
			if (value != "" && !pattern.test(value)) {
				alert(title + " must be entered as a number only.");
				if (isFocus) {	$element.attr('tabIndex', -1).focus();	}
				formChk = false;
				return false;
			}
		...
		if ( $element.attr('alpah') == 'alpah') {
			...
			if (!pattern.test(value)) {
				alert(title + " must be entered in English only.");
				if (isFocus) {	$element.attr('tabIndex', -1).focus();	}
				formChk = false;
				return false;
			}
		...
		if ( $element.attr('alphanum') == 'alphanum') {
			...
			if (!pattern.test(value)) {
				alert(title + " must be entered in mixed alphanumeric characters.");
				if (isFocus) {	$element.attr('tabIndex', -1).focus();	}
				formChk = false;
				return false;
			}
		...
		if ( $element.attr('matchedlength') > 0 && $element.attr('matchedlength') != valueLen) {
			alert(title + " is " + $element.attr('matchedlength') + " characters.");
			if (isFocus) {	$element.attr('tabIndex', -1).focus();	}
			formChk = false;
			return false;
		}
...
function setboardGrid(dataSet) {
	...
	if (dataSet == null || dataSet == "") {
		alert("There is no registered data.");
		return;
	} else {
